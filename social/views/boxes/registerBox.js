...
export default class RegisterBox extends React.Component{
  ...
  render(){
    ...
    return(
      <>
      <form class="FRBox" onSubmit={this.updateAvatarAndNameUser}>
        {this.state.isUploading && <ProgressBar progress={this.state.progress}/>}
        <div className="ImageContainerUpload">
          {!this.state.avatarURL && ( user.photoURL && <img style={{ objectFit : "cover"}} width="auto"  height="300px" src={user.photoURL}/>)}
          {this.state.avatarURL && <img style={{ objectFit : "cover"}} width="auto"  height="300px" src={this.state.avatarURL} />}
        </div>
